...
class ImageDataProvider(BaseDataProvider):
    ...
    def __init__(self, search_path, a_min=None, a_max=None, data_suffix=".tif", mask_suffix='_mask.tif', shuffle_data=True):
        ...
        if '.jpg' in label_path.lower():
            mask = self._load_jpg_file(label_path, np.bool)
        else:
    ...
    def _load_jpg_file(self, path, dtype=np.bool):
        # jpg only used for binary segmentation
        # assumes any color is label of 1
        # grab first channel only
        img_o = np.array(Image.open(path), dtype)
        label = img_o[:, :, 0]
        label = label.squeeze()
        return label
    ...
    def _next_data(self):
        ...
        if '.jpg' in label_name.lower():
            label = self._load_jpg_file(label_name, np.bool)
        else:
