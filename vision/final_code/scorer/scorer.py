...
class mAPScorer():
  ...
  def Pascal_mAP(self, GT_data, pred_data):
    ...
    for img_key in list(GT_data.keys()):
      ...
      if (len(pred_box) > 0):
        tp_fp_cf_i = self.metrics(np.array(GT_box), np.array(pred_box), 0.5)
        tp_fp_cf += tp_fp_cf_i
  ...
  def metrics(self, truth_boxes, test_boxes, IOU_threshold):
    ...
    for truth_box in truth_boxes:
      # for each ground truth box, compute max IOU, and check if maxIOU>threshold,
      # assign the corresponding box to gt box
      truth_poly = self.create_poly(truth_box)
      box_id = 0
      for test_box in test_boxes:
        # if box is already assigned, tp_fp_cf[box_id,1]=0, so dont use it
        iou_box = self.calculate_iou(truth_poly, self.create_poly(test_box[0:8]))
  ...
  def map_eleven(self, tp_fp_cf, total_truths):
    ...
    tp_fp_cf = np.array(sorted(tp_fp_cf, key=lambda x: x[-1], reverse=True))
