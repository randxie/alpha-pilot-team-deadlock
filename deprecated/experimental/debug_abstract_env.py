...
class AbstractEnv(gym.Env):
  ...
  def __init__(self, start_time, config=DEFAULT_CONFIG):
    ...
    #print("Initial position = {}".format(self.init_pose))
    self.euler_angle_offset = tf.transformations.euler_from_quaternion(self.init_pose[3:])
    #print("Euler angle offset = {}".format(self.euler_angle_offset))

    self.xyz_offset = self.init_pose[0:3]
    #print("XYZ offset = {}".format(self.xyz_offset))

    self.height_offset = self.xyz_offset[2]  # default offset for the quad
    self.target_gate = 10

    # for velocity before VINS
    self.z_prev = 0

    init_vel = np.array([0, 0, 0])
    init_rates = np.array([0, 0, 0])
    self.states = np.hstack((self.xyz_offset, self.euler_angle_offset, init_vel, init_rates))
    print("Initial states: {}".format(self.states))
